...
const changed = () => { if (notifyTimer) return; notifyTimer = setTimeout(() => { notifyTimer = null; if (window && !window.isDestroyed()) window.webContents.send('shellmate:changed'); }, 40); };
...
const schemas: Record<string, z.ZodTypeAny> = {
  snapshot: z.tuple([]), chatContext: z.tuple([id,string]),
  createWorkspace: z.tuple([string]), renameWorkspace: z.tuple([id,string]), setActiveWorkspace: z.tuple([id]),
  createConversation: z.tuple([]), renameConversation: z.tuple([id,string]), setTarget: z.tuple([id,id,z.enum(['disabled','ask','autonomous']).nullable()]),
...
if (!app.requestSingleInstanceLock()) app.quit();
  ...
  void app.whenReady().then(async () => {
    ...
    const terminal = new TerminalManager(store, secrets, changed, event => { if (window && !window.isDestroyed()) window.webContents.send('shellmate:terminal', event); });
    ...
    const snapshot = async (): Promise<Snapshot> => {
      ...
      return { workspaces: store.workspaces(), activeWorkspaceId: workspaceId, workspaceConnectionIds: store.workspaceConnectionIds(workspaceId), connections,
        conversations: store.conversations(workspaceId), targets: store.conversations(workspaceId).flatMap(conversation => store.targets(conversation.id)), messages: store.messages(workspaceId),
    ...
    const handlers: Record<string, (...args: any[]) => unknown> = {
      snapshot, chatContext: (conversationId, draft) => chat.chatContext(conversationId, draft),
      ...
      setTarget: (conversationId, connectionId, access) => store.setTarget(conversationId, connectionId, access),
      ...
      setWorkspaceConnection: (connectionId, included) => { if (!included) terminal.disconnect(activeWorkspace(), connectionId, true); store.setWorkspaceConnection(activeWorkspace(), connectionId, included); },
    ...
    window = new BrowserWindow({ width: 1500, height: 960, minWidth: 950, minHeight: 650, title: 'Shellmate', show: false, backgroundColor: '#191c1d', webPreferences: {
    ...
    diagnostics.record('app.started', { version: app.getVersion(), packaged: app.isPackaged });
    app.on('window-all-closed', () => app.quit());
    app.on('before-quit', event => {
      if (quitting) return; event.preventDefault(); quitting = true;
      void Promise.all([chat.shutdown(), auth.cancelLogin()]).finally(() => { terminal.shutdown(); if (notifyTimer) clearTimeout(notifyTimer); store.close(); app.quit(); });
    });
