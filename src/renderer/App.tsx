...
function ProvidersDialog({ snapshot, onClose, act }: { snapshot: Snapshot; onClose: () => void; act: (action: () => Promise<unknown>) => Promise<unknown> }) {
  ...
  const saveModel = async () => { const saved = await act(() => api().saveChatModel({ id: modelId, providerId, slug, efforts: efforts.split(',').map(x => x.trim()).filter(Boolean), contextLimit: Number(contextLimit) })); if (saved === null) return; setModelId(undefined); setSlug(''); setEfforts(''); setTest(''); setContextLimit(String(DEFAULT_CONTEXT_LIMIT)); };
  ...
  return <div className="modal-backdrop" role="presentation"><div className="dialog providers-dialog" role="dialog" aria-modal="true" aria-label="Chat providers">
    <div className="dialog-head"><h2>Chat providers</h2><button onClick={onClose} aria-label="Close">×</button></div><div className="dialog-body">
      ...
      <section className="settings-section"><h3>API models</h3>{snapshot.providers.chatModels.filter(model => !model.builtIn && !model.id.startsWith('codex:')).map(model => <div className="settings-row" key={model.id}><span><strong>{model.label}</strong><small>Efforts: {model.efforts.join(', ') || 'default'} · Context: {model.contextLimit.toLocaleString()} tokens · Built-in web search: {model.hostedSearch ? 'yes' : 'no'}</small></span><span className="fill" /><button onClick={() => { setModelId(model.id); setProviderId(snapshot.providers.providers.find(p => p.label === model.providerLabel)?.id ?? ''); setSlug(model.slug); setEfforts(model.efforts.join(', ')); setContextLimit(String(model.contextLimit)); setTest(''); }}>Edit</button><button className="danger" onClick={() => void act(() => api().deleteChatModel(model.id))}>Delete</button></div>)}
